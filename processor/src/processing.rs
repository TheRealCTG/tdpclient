...
pub async fn get_accomodation_handler(data: web::Data<AppState>) -> impl Responder {
 ...
 async fn  get_accomodations(appdata: web::Data<AppState>) -> Result<String, ProcessorCallError> {
    ...
    let  max_no_of_supplier: u64 = rng.gen_range(appdata.min_noofsuppliers_forrandomness..appdata.max_noofsuppliers_forrandomness);
   ...
   // println!("get_accomodations called=>enumerate suppliers {}",  suppliers.len());  
    ...
    for  supplier in suppliers.iter(){      
        let url = appdata.adaptorhosturl.clone();
         ...
         let handle = tokio::task::spawn(async move {          
             let data = match  get_accommodation_by_supplier(isupplierdata, url).await {
                 Ok(data) => {
                    data
                 },
                 Err(error) => { 
    ...
    tokio::task::spawn_blocking( move || {
        simulate_cpu_usage( &arc_appdata.firstsupplierdata, arc_appdata.max_cpu_usage_in_milliseconds,arc_appdata.min_cpu_usage_in_milliseconds); 
    }).await.unwrap();
   
   Ok(resultdata.to_string())
}
 //
 pub async fn get_accommodation_by_supplier(supplier_id: i32,ref hostname:String) -> Result<String, ProcessorCallError>  {
